...
class Bowler_comp():
            ...
            def calculateb(self,leagues,overs1,BatterType,Season,limit):
                    ...
                    for player in players:
                        ...
                        balls=len(self.df.loc[(self.df['is_wide']==0)  & (self.df['is_noball']==0) & (self.df["LeagueName"].isin(leagues))   & (self.df["BattingType"].isin(BatterType)) & (self.df["overs"].isin(overs))  & (self.df["Season"].isin(Season)) & (self.df['BowlingTeam'] == player) ] )
                        out = len(   self.df.loc[(self.df["BowlingTeam"] == player) & (self.df["LeagueName"].isin(leagues))  & (self.df["player_out"]==1)  & (self.df["BattingType"].isin(BatterType))  & (self.df["overs"].isin(overs)) & (self.df["Season"].isin(Season))])
                        ...
                        dots=len(self.df.loc[(self.df["BowlingTeam"] == player) & (self.df["LeagueName"].isin(leagues))  & (self.df["Extras_Run"]==0) & (self.df["batsman_run"]==0) & (self.df["BattingType"].isin(BatterType)) & (self.df["overs"].isin(overs))  & (self.df["Season"].isin(Season))  ] )
...
# print(bowcomp.calculateb(['TNPL'],[1,2,3,4],['RHB','LHB'],[2023],0).head(10))
#
#
#
#
# with open('C:/Users/adith/Documents/ds/t20_leagues/set_3_leagues/app/teams/bowling_comp/bowling_comp.pkl', 'wb') as f:
